...
client = OpenAI(
    ...
    api_key="f2da4a92-1ce6-4376-b33a-b6124d686c9b" # Replace with your actual API key
)
...
def extract_between_regex(text, start_marker, end_marker):
    pattern = re.escape(start_marker) + r"(.*?)" + re.escape(end_marker)
    match = re.search(pattern, text, re.DOTALL)
    return match.group(1).strip() if match else None

result = extract_between_regex(str(chat_completion.to_dict()), "```json", "```")
